...
function _times(each, times, fn) {
    ...
    var arr = Array.apply(null, Array(times)).map(function (_, i) {
        return i;
    });
    return each(arr, fn);
}
aplus.times = function times(times, fn) {
    return _times(aplus.each, times, fn);
};
aplus.timesSeries = function timesSeries(times, fn) {
    return _times(aplus.eachSeries, times, fn);
};
aplus.timesLimit = function timesLimit(times, limit, fn) {
