...
aplus.queue = function queue(worker, concurrency) {
    ...
    function taskify(tasks) {
        ...
        tasks.forEach(function (task) {
            var fn, promise = new aplus.Promise(function (resolve, reject) {
                fn = function () {
                    return _thenable(worker, [task]).then(resolve, reject);
                };
            });
    ...
    function next() {
        ...
        while (workers < q.concurrency && queueTasks.length > 0) {
            var task = queueTasks.shift();
            workers += 1;
            task().then(after, after);
    ...
    q = {
        ...
        push: function push(tasks) {
            return addTasks([].push, tasks);
        },
        unshift: function unshift(tasks) {
            return addTasks([].unshift, tasks);
        },
        saturated: noop,
        empty: noop
    };
