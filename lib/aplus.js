    ...
    return new aplus.Promise(function (resolve, reject) {
        ...
        function spawn() {
            while (!done && started < keys.length && active < limit) {
                ...
                active += 1;
                started += 1;
                _thenable(iterator, [arr[keys[index]], keys[index], arr]).then(getCompleter(index)).catch(onReject);
            }
        }
        spawn();
    });
...
aplus.forever = function forever(task) {
    return new aplus.Promise(function (resolve, reject) {
        function runner() {
            return _thenable(task).then(runner);
        }
...
aplus.during = aplus.whilst = function whilst(test, fn) {
    function runner() {
        return _thenable(test).then(function (value) {
            if (value) {
                return _thenable(fn).then(runner);
            }
            return aplus.Promise.resolve(undefined);
        });
    }
